...
export interface RecastVideoOptions {
   ...
   * Pixel-sharp up to zoom `scale x viewport width / output width`, so pick
   * `output width x max zoom / viewport width` (2.4 for 1.8x at 1440p from
   * 1920). Costs grow with the pixels (scale²), the render less than that:
   * 2 -> 2.4 is 44% more pixels and rendered 25% slower in one measurement.
   * Default 2.
   */
  scale?: number
}
