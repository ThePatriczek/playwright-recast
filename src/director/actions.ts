import { constrainPose, focusInShot, overview, type CameraKeyframe, type CameraPose, type FocusRegion } from './camera.js'
...
export function planAction(action: CameraAction, pose: CameraPose, context: ActionContext, atMs: number, endMs: number): ActionPlan {
  ...
  switch (action) {
    ...
    case 'focus': return target ? move(frameTargets([target], context.maxZoom ?? 1.45), [target], 'frame-subject') : hold('missing-target')
    case 'fit': {
      const targets = context.related.flatMap(region => visibleRegion(region) || [])
      return targets.length >= 2 ? move(frameTargets(targets, context.maxZoom ?? 1.45), targets, 'frame-related-subjects') : hold('missing-related-targets')
    }
    case 'reveal': {
      const result = context.result && visibleRegion(context.result)
      return result ? move(frameTargets([result], Math.min(context.maxZoom ?? 1.45, 1.35)), [result], 'show-observed-result') : hold('missing-result')
    }
    case 'follow': {
      ...
      for (const sample of samples) {
        ...
        const desired = frameTargets([region], Math.min(context.maxZoom ?? 1.45, Math.max(pose.zoom, 1.45)))
