...
Auto-detected clicks time the cursor approach off the trace. For clicks that wait on a page load, the cursor can start gliding before the target paints — it appears to move over a still-loading screen. The [`click()` / `markClick()`](/docs/helpers/click) test helpers fix this by marking the click explicitly.
...
A marker that lands near an auto-detected click suppresses it, so you never get a duplicate ripple. With a `voiceover()` stage, the hold also extends the audio and subtitles so narration stays in sync across held clicks. See the [`click()` helper reference](/docs/helpers/click) for details.
...
- [`click()` / `markClick()` helpers](/docs/helpers/click) -- mark clicks for a polished, held cursor approach
