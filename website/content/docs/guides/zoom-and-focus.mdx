...
Targets only a few pixels apart, such as consecutive lines of one code block, make the camera sway between them. `panStabilizationThreshold` keeps it still: per axis, it pans only once the next target is more than that fraction of the crop's size away from what is shown.

```typescript
.render({ resolution: '1440p', zoom: { panStabilizationThreshold: 0.3 } })
```

Each axis decides on its own, so a target further down but at the same indentation pans vertically only. A change of zoom level always moves the camera. Applies to every zoom source: `zoom()`, `autoZoom()` and report data.
...
## Keeping zoomed text sharp

A zoom enlarges source pixels. To stay sharp, record at `output size x zoom` or more, e.g. with `recastVideo()` from `playwright-recast/config` - see [Sharp text when zooming](/docs/pipeline/parse#sharp-text-when-zooming). The renderer warns when it has to upscale.
