...
// Settle → optional hover dwell → mark → real click.
// Options pass straight to Playwright.
...
3. When enabled, hover the locator and wait the **dwell** (`hoverDwellMs`, default 0 ms) so the app paints its own hover state. The hover is best effort and capped at one second; failure skips the dwell.
4. `markClick(locator)` — write the marker at the element center after the dwell, keeping it close enough to pair with the real click.
5. `await locator.click(options)` — the real click.

The settle is the only real-time cost added by default. Tune or disable it globally with [`setupRecast(test, { clickSettleMs })`](/docs/helpers/setup-recast#clicksettlems--the-click-settle). To record hover-only UI, opt into [`setupRecast(test, { hoverDwellMs })`](/docs/helpers/setup-recast#hoverdwellms--capture-the-hover-state). Pass `0` to disable either wait.
