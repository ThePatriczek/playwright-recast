...
Both helpers need `setupRecast(test)` and a pipeline that renders clicks — [`.cursorOverlay()`](/docs/pipeline/cursor-overlay) and/or [`.clickEffect()`](/docs/pipeline/click-effect). Without either stage the marker is recorded but nothing draws it.
