...
| `opts.duration` | `number \| 'narration'` | `2000` | Visibility duration in milliseconds, or `'narration'` to end with the narration the mark belongs to |
| `opts.fadeOut` | `number` | `0` | Fade out duration in milliseconds |
...
## Lasting exactly as long as the narration

With `duration: 'narration'` the mark ends when its narration's audio ends, so it neither clears mid-sentence nor lingers into the next action. It belongs to the narration playing when it appears, else to the next one, so it can be set just before the `narrate()` it illustrates. Without a voiceover it falls back to the default duration.

```typescript
await highlight(line, { duration: 'narration' })
await narrate('This line mounts the model stage.')
