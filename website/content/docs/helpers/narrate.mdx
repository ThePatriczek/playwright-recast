...
> **With TTS you usually don't need `autoWait`.** The [`voiceover()`](/docs/pipeline/voiceover) stage already stretches the video to fit each line — see [Voiceover-driven freezes](#voiceover-driven-freezes) and [`waitForNarration()`](/docs/helpers/wait-for-narration#pacing-narration-without-autowait). Reach for `autoWait` mainly when rendering without TTS, where there is no audio to size the pause.
...
To force a hold at a deliberate point — e.g. before a click that must not be talked over, or at the end of a scenario — call [`waitForNarration()`](/docs/helpers/wait-for-narration). It ends the preceding narration's window there so the renderer holds until that line is fully spoken.
