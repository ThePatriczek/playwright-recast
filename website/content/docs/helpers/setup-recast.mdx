...
`setupRecast(test)` connects the playwright-recast step helpers (`narrate`, `zoom`, `pace`, `highlight`, `click`, `markClick`, `waitForNarration`) to your Playwright test context. Without this call, the helpers cannot access the current test's annotation system and will throw an error.
