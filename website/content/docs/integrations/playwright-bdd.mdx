...
setupRecast(test, { typingDelayMs: 100 })
export { narrate, pace, typeText }
```

`setupRecast(test)` connects the step helpers to the Playwright test context. Call it once before defining any steps that rely on narration, zoom, highlight, click, or narration-boundary markers. Its optional `typingDelayMs` sets the suite-wide average speed for [`typeText()`](/docs/helpers/type-text).
...
```typescript
import { narrate, zoom, typeText, highlight, pace } from 'playwright-recast'
...
When('the user fills in the search', async ({ page }, docString?: string) => {
  ...
  await typeText(searchInput, 'quarterly report')
...
Swap `page.click()` for the [`click()` helper](/docs/helpers/click) on the interactions you want to emphasise — the renderer gives those a deliberate, held cursor approach. Use [`waitForNarration()`](/docs/helpers/wait-for-narration) to hold the video until a line finishes speaking before the next step runs:
...
With a `.voiceover()` stage you don't need `autoWait` or `pace()` for timing: the rendered video freezes at each `waitForNarration()` for exactly as long as the line needs, so the test can run at full speed and the narration still stays in sync. See [Pacing narration without autoWait](/docs/helpers/wait-for-narration#pacing-narration-without-autowait).
