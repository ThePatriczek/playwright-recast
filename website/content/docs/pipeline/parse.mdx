...
The page still lays out at 1920x1080, but the video is recorded at 3840x2160. `recastVideo()` sets the three things that have to agree, each of which fails silently on its own: `--force-device-scale-factor` (headless Chromium ignores `deviceScaleFactor` for the video), `video.size` of viewport x scale (Playwright pads a smaller frame with gray instead of scaling it), and headless mode (headed Chromium follows the OS display scale). Decimal scales work: for a sharp 1.8x zoom into 1440p, `scale: 2.4` records 4608x2592. The scale is rounded up to the next one that gives whole, even device pixels (1.3334 records at 1.35 for 1920x1080), since only those record without gray padding or rescaling. In general, `scale = max(1, max zoom x max(output width / viewport width, output height / viewport height))`. Chromium only.
