...
- **Audio-overflow freezes** — when a narration's audio is longer than its visual window (often at a [`waitForNarration()`](/docs/helpers/wait-for-narration) boundary), the renderer holds the last frame until the line finishes.
- **Click approach holds** — the [`click()` helper](/docs/helpers/click) freezes the painted frame so the cursor can glide in (`approachMs`). When voiceover is present these holds are folded into the same audio/subtitle shift, so spoken lines after a held click stay aligned.
